#!/usr/bin/env python3
"""
Grid search over XGBRegressor hyperparameters for the AST (assists) model.
...
def load_and_prepare_data(csv_path: str) -> tuple:
    """Load and preprocess data to match the AST notebook."""
    pbp_df = pd.read_csv(csv_path)
    pbp_df["assists_remaining"] = pbp_df["final_assists"] - pbp_df["current_assists"]
    pbp_df = pbp_df.drop(
        columns=[
            "player_id",
            "game_id",
...
def main():
    ...
    print(f"AST Grid search: {n_combos} combinations")
    print("=" * 80)
