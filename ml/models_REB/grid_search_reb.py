#!/usr/bin/env python3
"""
Grid search over XGBRegressor hyperparameters for the REB (rebounds) model.
...
def load_and_prepare_data(csv_path: str) -> tuple:
    """Load and preprocess data to match the REB notebook."""
    pbp_df = pd.read_csv(csv_path)
    pbp_df["rebounds_remaining"] = pbp_df["final_rebounds"] - pbp_df["current_rebounds"]
    pbp_df = pbp_df.drop(
        columns=[
            "season",
            "game_id",
...
def main():
    ...
    print(f"REB Grid search: {n_combos} combinations")
    print("=" * 80)
